<x-filament-widgets::widget class="w-full">
    <div style="display: flex; flex-wrap: wrap; gap: 0.75rem; width: 100%; max-width: 100%;">
        @forelse($this->getAgents() as $agent)
            <div style="width: calc(16.666% - 0.625rem); display: flex; align-items: flex-start; gap: 1.5rem; border: 2px solid #e5e7eb; border-radius: 0.5rem;" class="shadow p-4 transition-all duration-200 {{ $agent->is_logged_in ? 'bg-green-50' : 'bg-gray-50' }}" data-agent-id="{{ $agent->id }}">
                <!-- Left: User Avatar -->
